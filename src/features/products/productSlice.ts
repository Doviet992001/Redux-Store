...
export const getImageBySku = (sku: number, listImages: string[]) => {
    let img = ""
    for (let i = 0; i < listImages.length; i++) {
        if (listImages[i].indexOf(sku.toString()) > -1) {
            img = listImages[i]
        }
    }
    // const img =  listImages.find(i => i.includes(sku.toString()))
    return img;
}

// export const getProductBySize = (availableSizes: string[], selectSize: string[]) => {
...
export const productSlice = createSlice({
    ...
    reducers: {
        ...
        //     const filteredSize = state.products.filter((product) => {
        //         product.availableSizes.includes(action.payload)
        //     });
        //     return {
        //         ...state,
        //         selectSize: 
        //         action.payload.length  > 0 ? filteredSize : [...state.products]
        //     }
        // }
    },
    extraReducers: (builder) => {
        builder.addCase(
            ...
            (state, action: PayloadAction<{ products: IProduct[] }>) => {
                ...
                const result: ProductState = { products: [] }
                products.forEach((p) => {
...
// export const {searchBySize} = productSlice.actions
